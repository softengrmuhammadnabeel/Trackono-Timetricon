...
const Switch = () => {
  ...
  React.useEffect(() => {
    if (theme === 'system') {
      ...
      decideThemeValues(systemTheme);
    } else {
      decideThemeValues(theme as Theme.Light | Theme.Dark);
    }
  }, [theme]);
    ...
    <StyledWrapper>
      <div className="container">
        <label className="switch">
          <input className="togglesw" type="checkbox" checked={inputValue}
...
@media (max-width: 768px) {
  .switch {
    --width: 3rem; /* Smaller width for tablets */
...
@media (max-width: 480px) {
  .switch {
    --width: 2.5rem; /* Smaller width for mobile phones */
    --height: calc(var(--width) / 2.5);
  }
}
  .container {
